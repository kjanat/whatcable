...
extension ChargingDiagnostic {
    public init?(
        ...
        if let cableW = cableMaxW, cableW < chargerMaxW {
            ...
            self.summary = String(
                localized: "Cable is limiting charging speed", bundle: _coreLocalizedBundle)
            self.detail = String(
                localized:
                    "Charger can deliver up to \(chargerMaxW)W, but this cable is only rated to carry \(cableW)W. Replace the cable to charge faster.",
            ...
            cableMaxW.map { n < $0 - max(5, $0 / 10) } ?? true
        {
            ...
            self.summary = String(
                localized: "Charging at \(n)W (charger can do up to \(chargerMaxW)W)",
                bundle: _coreLocalizedBundle)
            self.detail = String(
                localized:
            ...
            if batteryFullyCharged == true {
                ...
                self.summary = String(
                    localized: "Battery full, not charging", bundle: _coreLocalizedBundle)
                self.detail = String(
                    localized:
                        "Charger and cable are fine. The Mac will draw up to \(n)W when it needs to.",
                    bundle: _coreLocalizedBundle)
            } else {
                self.summary = String(
                    localized: "Charging well at \(n)W", bundle: _coreLocalizedBundle)
                self.detail = String(
                    localized: "Charger and cable are well-matched.", bundle: _coreLocalizedBundle)
            }
        } else if isAdapterFallback {
            self.bottleneck = .chargerLimit(chargerW: chargerMaxW)
            self.summary = String(
                localized: "System reports charger at \(chargerMaxW)W", bundle: _coreLocalizedBundle
            )
            self.detail = String(
                localized:
            ...
            self.bottleneck = .chargerLimit(chargerW: chargerMaxW)
            self.summary = String(
                localized: "Charger advertises up to \(chargerMaxW)W", bundle: _coreLocalizedBundle)
            self.detail = String(
                localized: "Negotiation hasn't completed yet.", bundle: _coreLocalizedBundle)
        }
    }
}
